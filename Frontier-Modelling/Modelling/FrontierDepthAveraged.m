function classifiers=FrontierDepthTest(T, layerInfo, alpha, percentile, show, outlier, outlier2, MapInfo)
...
percentileSize = round(percentile*nonBoundPointsSize/100);

% Creating an empty array to determine each pixel's depth
intensity = NaN(percentileSize,length(idx));

for ex=1:percentileSize

% Selecting the sample's radius as the percentile's value of the distances
radiusClass = prctile(radius,ex*100/nonBoundPointsSize,2);
...
for i=1:length(idx)
    ...
    intensity(ex,i) = sum(response<=radiusClass)/nonBoundPointsSize;
end
end
